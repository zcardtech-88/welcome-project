...
const sections = [
  ['specs','Vehicle Specifications','▣'], ['highlights','Highlights','★'], ['description','Description','≡'], ['gallery','Gallery','▧'], ['video','Featured Video','▶'], ['seller','Seller','●'], ['social','Social Links','↗']
]
...
export default function AutomotiveCard(){
  ...
  const actions = [['☎','Call'],['◉','WhatsApp'],['⌖','Location'],['↗','Share'],['▣','QR Code']]
  return <main className="page-shell"><div className="card-wrap">
    ...
    <div className="actions">{actions.map(([icon,label])=><button key={label} aria-label={label}><i>{icon}</i><span>{label}</span></button>)}</div>
    <div className="sections">{sections.map(([id,title,icon])=> <section key={id} className="accordion">
      <button className="accordion-head" onClick={()=>toggle(id)}><span className="section-title"><i>{icon}</i><b>{title}</b></span><span className={open===id?'rot':''}>⌄</span></button>
      {open===id && <div className="accordion-body">
        ...
        {id==='video' && <div className="video"><img src={vehicle.video_thumbnail} alt="Featured vehicle video"/><div className="play">▶</div></div>}
        {id==='seller' && <div className="seller"><div className="avatar">AH</div><div className="seller-info"><h3>{vehicle.seller_name}{vehicle.seller_verified && <span>✓</span>}</h3><p>{vehicle.seller_title}</p><p>{vehicle.seller_company}</p></div><div className="seller-actions"><button>☎ Call</button><button>◉ WhatsApp</button><button>✉ Email</button></div></div>}
        {id==='social' && <div className="socials"><a href={vehicle.website}>W</a><a href={vehicle.instagram}>IG</a><a href={vehicle.facebook}>F</a><a href={vehicle.youtube}>YT</a><a href={vehicle.linkedin}>in</a></div>}
      </div>}
    </section>)}</div>
