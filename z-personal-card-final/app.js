...
function t(key) { return copy[state.lang][key] || copy.en[key] || copy.ar[key] || key; }
...
function applyLanguage() {
  ...
  el.intl.forEach((node) => { const key = node.dataset.i18n; if (key) node.textContent = t(key); });
  el.placeholders.forEach((node) => { node.placeholder = t(node.dataset.i18nPlaceholder); });
}
...
function sanitizeUrl(value) { const v = String(value || '').trim(); if (!v) return ''; if (/^https?:\/\//i.test(v) || /^mailto:/i.test(v) || /^tel:/i.test(v) || /^wa\.me/i.test(v)) return v; return `https://${v.replace(/^\/+/, '')}`; }
function setText(node, value) { if (!node) return; const text = String(value || '').trim(); node.textContent = text; node.classList.toggle('hidden', !text); }
function setImage(node, src) { if (node) node.src = src || defaultState.profile_image; }
function setActionHref(btn, href) { if (!btn) return; btn.dataset.href = href || ''; btn.classList.toggle('hidden', !href); }

function renderSocials() {
  const socialFields = [['linkedin', 'in'], ['instagram', 'ig'], ['facebook', 'f'], ['x', 'x'], ['youtube', 'yt']];
  const linkFields = [['website', 'web'], ['portfolio', '⌁']];
  el.socialRow.innerHTML = '';
  socialFields.forEach(([field, label]) => { const url = sanitizeUrl(state.social_links[field]); if (!url) return; const b = document.createElement('button'); b.type = 'button'; b.textContent = label; b.title = url; b.dataset.edit = field; el.socialRow.appendChild(b); });
  el.linkRow.innerHTML = '';
  linkFields.forEach(([field, label]) => { const url = sanitizeUrl(state[field]); if (!url) return; const b = document.createElement('button'); b.type = 'button'; b.textContent = label; b.title = url; b.dataset.edit = field; el.linkRow.appendChild(b); });
}

function updateStickyMini() { const compact = window.scrollY > 90; el.stickyMini.classList.toggle('show', compact); el.zCard.classList.toggle('compact', compact); }
function updateDockState() { document.getElementById('dockCall').classList.toggle('hidden', !normalizePhone(state.phone_number)); document.getElementById('dockWa').classList.toggle('hidden', !normalizePhone(state.whatsapp_number)); }
function makeQrUrl() { const url = `${location.origin}${location.pathname}?card=${encodeURIComponent(state.card_id)}`; return `https://api.qrserver.com/v1/create-qr-code/?size=240x240&margin=0&data=${encodeURIComponent(url)}`; }
function renderQR() { el.qrImage.src = makeQrUrl(); }

function updateCard() {
  setImage(el.coverImage, state.cover_image); setImage(el.profileImage, state.profile_image); setImage(el.miniAvatar, state.profile_image);
  setText(el.nameField, state.full_name); setText(el.titleField, state.job_title); setText(el.companyField, state.company_name); setText(el.locationField, state.location); setText(el.bioField, state.short_bio);
  setText(el.statusBadge, state.status || t('tap_to_edit')); setText(el.miniName, state.full_name); setText(el.miniTitle, state.job_title);
  setActionHref(el.callBtn, `tel:${normalizePhone(state.phone_number)}`); setActionHref(el.waBtn, state.whatsapp_number ? `https://wa.me/${normalizePhone(state.whatsapp_number)}` : ''); setActionHref(el.emailBtn, state.email ? `mailto:${state.email}` : '');
  renderSocials(); renderQR(); updateDockState(); persistState();
}

function toast(message) { el.toast.textContent = message; el.toast.classList.add('show'); clearTimeout(window.__toastTimer); window.__toastTimer = setTimeout(() => el.toast.classList.remove('show'), 2200); }
...
function getFieldValue(field) { return field in state ? state[field] : (state.social_links?.[field] || ''); }

function openEditor(field) {
  activeField = field; pendingImageField = '';
  ...
  el.editorLabel.textContent = t('eyebrow'); el.editorTitle.textContent = info.title; el.editorHelp.textContent = info.help;
  el.imageUploadGroup.hidden = !isImage; el.secondaryInputGroup.hidden = !isImage; el.editorInputGroup.hidden = isImage || isBio || isQR || isSave; el.textareaGroup.hidden = !isBio; el.qrPreviewGroup.hidden = !isQR;
  el.editorInputLabel.textContent = info.title; el.editorInput.value = !isImage && !isBio && !isQR && !isSave ? value : ''; el.editorInput.placeholder = t('type_here'); el.editorInput.type = field === 'phone_number' || field === 'whatsapp_number' ? 'tel' : 'text';
  el.editorTextLabel.textContent = info.title; el.editorTextArea.value = isBio ? value : ''; el.editorTextArea.placeholder = t('write_bio');
  el.secondaryInputLabel.textContent = isImage ? (state.lang === 'ar' ? 'رابط الصورة' : 'Image URL') : ''; el.secondaryInput.value = isImage ? value : ''; el.secondaryInput.placeholder = 'https://...';
  el.imageFile.value = ''; el.qrImage.src = makeQrUrl();
  if (isSave) { el.editorTitle.textContent = state.lang === 'ar' ? 'حفظ جهة الاتصال' : 'Save Contact'; el.editorHelp.textContent = state.lang === 'ar' ? 'يمكنك تنزيل بطاقة vCard من بياناتك الحالية.' : 'Download a vCard using your current data.'; }
  if (isQR) { el.editorTitle.textContent = t('qr'); el.editorHelp.textContent = t('editor').qr_code.help; }
  openModal(el.editorModal);
}

function downloadVCard() {
  const lines = ['BEGIN:VCARD', 'VERSION:3.0', `FN:${state.full_name}`, `TITLE:${state.job_title}`, `ORG:${state.company_name}`, `TEL;TYPE=CELL:${normalizePhone(state.phone_number)}`, `EMAIL:${state.email || ''}`, 'END:VCARD'];
  const blob = new Blob([lines.join('\n')], { type: 'text/vcard;charset=utf-8' });
  const url = URL.createObjectURL(blob); const a = document.createElement('a'); a.href = url; a.download = `${state.full_name || 'z-card'}.vcf`; document.body.appendChild(a); a.click(); a.remove(); URL.revokeObjectURL(url);
...
async function shareCurrentCard() {
  ...
  try { if (navigator.share) return void await navigator.share(payload); await navigator.clipboard.writeText(shareUrl); toast(state.lang === 'ar' ? 'تم نسخ الرابط' : 'Link copied'); }
  catch { toast(state.lang === 'ar' ? 'المشاركة غير متاحة الآن' : 'Sharing is unavailable'); }
}
...
function handleAction(action) {
  if (action === 'call') { const phone = normalizePhone(state.phone_number); if (phone) window.location.href = `tel:${phone}`; else openEditor('phone_number'); return; }
  if (action === 'whatsapp') { const wa = normalizePhone(state.whatsapp_number || state.phone_number); if (wa) window.open(`https://wa.me/${wa}`, '_blank', 'noreferrer'); else openEditor('whatsapp_number'); return; }
  if (action === 'share') { openModal(el.shareModal); return; }
  if (action === 'qr') openEditor('qr_code');
}

function bindEvents() {
  el.langToggle.addEventListener('click', () => { state.lang = state.lang === 'ar' ? 'en' : 'ar'; applyLanguage(); updateCard(); persistState(); });
  el.themeToggle.addEventListener('click', () => { setTheme(state.theme === 'dark' ? 'light' : 'dark'); updateCard(); });

  document.addEventListener('click', (event) => {
    const target = event.target.closest('[data-edit], [data-action], [data-share], [data-qr], [data-close]');
    if (!target) return;
    const closeTarget = target.dataset.close; if (closeTarget) return void closeModal(document.getElementById(closeTarget));
    const action = target.dataset.action; if (action) return void handleAction(action);
    if (target.matches('[data-share]')) return void openModal(el.shareModal);
    if (target.matches('[data-qr]')) return void openEditor('qr_code');
    const field = target.dataset.edit; if (!field) return;
  ...
  el.imageFile.addEventListener('change', () => {
    const file = el.imageFile.files?.[0]; if (!file) return;
    const reader = new FileReader();
    reader.onload = () => { pendingImageField = String(reader.result || ''); if (activeField === 'cover_image') el.coverImage.src = pendingImageField; if (activeField === 'profile_image') el.profileImage.src = pendingImageField; toast(state.lang === 'ar' ? 'تم تحميل الصورة' : 'Image loaded'); };
    reader.readAsDataURL(file);
  });

  el.nativeShareBtn.addEventListener('click', shareCurrentCard);
  el.copyLinkBtn.addEventListener('click', async () => { const shareUrl = `${location.origin}${location.pathname}?card=${encodeURIComponent(state.card_id)}`; await navigator.clipboard.writeText(shareUrl); toast(state.lang === 'ar' ? 'تم نسخ الرابط' : 'Link copied'); });
  ...
  window.addEventListener('keydown', (event) => { if (event.key === 'Escape') { closeModal(el.editorModal); closeModal(el.shareModal); } });
}

function applyDataToDom() { applyLanguage(); setTheme(state.theme); updateCard(); window.__lastScrollY = window.scrollY; updateStickyMini(); }
function init() { applyDataToDom(); bindEvents(); }
